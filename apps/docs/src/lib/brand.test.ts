...
// The test lives with a consumer rather than in `packages/brand`, because
// that package claims no ambient types on purpose: it is data for two browser
// bundles, and giving it Node types to run one test would hand Node globals
// to a browser program. Both sites import these tokens, so either could host
// this; the documentation site does.
const brand = fileURLToPath(new URL("../../../../packages/brand/", import.meta.url))
const tokens = readFileSync(`${brand}tokens.css`, "utf8")
const motion = readFileSync(`${brand}motion.css`, "utf8")
const typography = readFileSync(`${brand}typography.css`, "utf8")
...
describe("the two typefaces", () => {
  ...
  // the brand package, and this test is what keeps them there.
  ...
  test("every declared font file exists", () => {
    const files = new Set(readdirSync(`${brand}fonts`))
