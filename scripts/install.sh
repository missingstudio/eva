...
#
# A channel is resolved here rather than stored anywhere. Nothing in this
# script can move a version.
#
...
# checksums.txt. Without cosign the install goes ahead on the checksum and
# says what that leaves unproven; --require-signature makes it a refusal
# instead.
#
# Windows is not covered. Use the archive from the release page, or npm.
#
# Two seams make this testable. `fetch` and `fetch_to` are the only things
# that reach the network, so tests replace them with recorded releases.
# `cosign_verify` and `have_cosign` are the second: a test cannot mint a
# sigstore bundle, so it substitutes the verdict and asserts what this script
# does with each one.
#
...
#
# It is read here as well as from the flag, so that a machine can decide once
# for every install on it rather than at every call site. install_main
# overrides it when --require-signature is given, and never the other way
# round: a flag may tighten this and nothing may loosen it.
...
# establishes there is a terminal to write it to. That is what lets each
# printf below be written once rather than twice, and it is what keeps a CI
# log clean: a run that never calls install_choose_style writes exactly what
# this script wrote before any of it had colour.
#
# Eva's mark is black and white and its palette has no accent, so this has
# none either. Weight carries the hierarchy: bold is a result, dim is a
# remark.
C_OFF=''
C_DIM=''
C_BOLD=''
...
# What the install proved, written where each fact is known and printed where
# a person can use it. See report_verified.
SIG_SUMMARY=''
SIG_NOTE=''
...
OPTIONS:
  ...
  --from-dist <dir>  install from a local `bun run rehearse` build rather than
                     a release, reaching no network. This is how the rehearsal
                     runs this script before there is a release.
  --require-signature
...
#
# It is asked per descriptor because the two answer differently: a run with
# its stdout in a file still draws its bar on a terminal. NO_COLOR is honoured
# because somebody who set it has answered this already, and a dumb terminal
# is one that cannot colour anything.
color_ok() {
...
# UTF-8 gets the ASCII bar and no wordmark, because three bytes written in
# hope arrive as three wrong characters.
unicode_ok() {
...
#
# stty is asked first, and asked on stderr, because `tput cols` on macOS
# answers with the terminfo entry's own 80 whatever the window is — which
# would wrap the wordmark on a narrow terminal and hide it on a wide one.
term_width() {
...
# install_choose_style fills the escapes in, when there is somewhere to put
# them.
...
#
# A person's own home directory is the longest and least telling part of a
# line they are about to read three times.
tilde() {
	case "${HOME:-}" in
	'') printf '%s' "$1" ;;
	*)
...
# scripts/release/context.ts, and the rehearsal asserts the two agree by
# running this script against the files a snapshot build actually wrote.
#
# The names carry no version: an archive is named by what it runs on, and the
# release's tag names the version.
archive_name() {
	case "$1" in
	darwin) printf 'eva-darwin-%s.zip' "$2" ;;
	*) printf 'eva-linux-%s.tar.gz' "$2" ;;
	esac
}
...
# Channels.
#
# Every path resolves one release document and then reads it. The document
# names the tag, and it names each asset's size — which is where the progress
# bar gets its total, at the cost of no extra request.
#
# The parsers are dependency-free, because jq is not on every machine that
# will run this. They are anchored on the indentation: a release's own fields
# are indented four spaces in the list and an asset's are indented eight, so
# nothing nested can be read as a release's own flag.
...
# release_doc <channel> <version> <path> writes the release document to
# <path>.
#
# One request for a named version, and one for the stable channel. Two for
# next, because the list says which release is the prerelease and the release
# itself says how big its assets are.
release_doc() {
	if [ -n "$2" ]; then
...
#
# installed_version <path to an eva binary> writes the version a release
# build reports, and nothing otherwise.
#
...
# report_start is the opening line: what is being done, and the facts that
...
# Progress.
#
# The bar watches the file the download writes. It asks the downloader
# nothing, so it serves curl and wget alike, and the seam above stays the only
# thing that reaches the network. A test that replaces fetch_to is not a
# terminal, so none of this runs there.
...
# stderr is the one to ask. This is what keeps a carriage return every tenth
# of a second out of a CI log.
progress_supported() {
	[ -t 2 ] && [ -z "${EVA_INSTALL_NO_PROGRESS:-}" ]
}

# progress_stop shows the cursor again, and writes nothing where no bar hid
# it.
#
...
# rather than an error inside the loop.
progress_delay() {
...
#
# The filled part is appended to and never rebuilt, because the bar only
# grows. That keeps the whole of the drawing inside the shell: the one process
# a frame costs is the one that read the size.
#
# The arithmetic is in kibibytes. Bytes times a hundred overflows a 32-bit
# shell at 21 MB, and an archive will reach that.
progress_draw() {
	total_kb=$(($2 / 1024))
...
# take, and it is capped: a bar spanning a wide window reads as a loading
# screen rather than as one line of a transcript.
progress_style() {
	if unicode_ok; then
		PROG_FULL='█'
		PROG_EMPTY='░'
...
#
# It stops when the file is whole, and when the process writing it is gone.
# The first of those is what makes it stop on every shell: a child that has
# exited and has not been reaped still answers kill -0.
#
# The last frame draws the size the file reached. A download that failed at
# 43% says 43%, because a bar that ends at 100% over a failure is a bar that
# lies.
progress_watch() {
	progress_style
	filled=''
...
#
# The plain fetch when there is no terminal, and when the size is unknown. A
# bar with a guessed denominator is worse than no bar.
...
#
# Functions for the reason fetch is: a test can neither install cosign nor
# mint a sigstore bundle, so it replaces the verdict and asserts what this
# script does with it.
...
# signature with no expected identity says only that somebody signed this, so
# a verification that pins neither is not a verification.
#
...
#
# Everything but a letter, a digit, an underscore, a slash, or a hyphen.
# GitHub allows a dot in a repository name, and an unescaped dot matches any
# character — so `evilXcom/eva` would satisfy an identity built for
# `evil.com/eva`.
regexp_escape() { printf '%s' "$1" | sed 's/[^A-Za-z0-9/_-]/\\&/g'; }

# signer_identity is the identity a release's signing certificate must carry.
#
# It is the release workflow, in this repository, running on main — because a
# release is a dispatch on main, and the certificate records the ref the
# workflow ran on:
#
#   URI:https://github.com/missingstudio/eva/.github/workflows/release.yml@refs/heads/main
#
...
signer_identity() {
	printf '^https://github\.com/%s/\.github/workflows/release\.yml@refs/heads/main$' \
		"$(regexp_escape "$REPO")"
}
...
# about a machine; what a person needs is what that leaves unproven. Neither
# is printed here — see report_verified for where they go and why.
unverified() {
...
verify_signature() {
	...
	if ! cosign_verify "$2" "$1" "$identity" "$SIG_ISSUER"; then
		# Two things fail here and cosign reports one status for both: the
		# file changed after it was signed, or it was signed by somebody else.
		# Which one is in cosign's own prose, and this does not parse a
		# dependency's prose to find out — so the message names both causes
...
repository's release workflow. Nothing was installed, and this is not a case
to retry: check it by hand before trusting these bytes.
  https://github.com/$REPO/blob/main/docs/reference/ci-cd.md"
	fi
...
#
# The archive format follows the platform: zip on macOS, tar.gz on Linux —
# the same case archive_name decides.
unpack_and_install() {
	case "$1" in
	*.zip) unzip -oq "$1" -d "$2" ;;
	*) tar -xzf "$1" -C "$2" ;;
...
# The wordmark.
#
# The letters are the eva logo sampled onto a character grid. That drawing is
# built on four horizontal bands, so only a multiple of four rows keeps its
# proportions: four rows gives each band one.
#
# It is a literal because a payoff that has to fetch something is a payoff
# that can fail, and it is drawn only where it fits whole: a wrapped mark is
# worse than none, so a narrower terminal gets nothing.
wordmark() {
	[ -t 1 ] || return 0
	unicode_ok || return 0

	# The blank line belongs to the mark and not to the bar above it, so that
	# a terminal too narrow to draw one is left with one blank line here and
	# not two.
	ww=$(term_width)
...
# read in: the signature is checked first so that the checksums are Eva's
# before they are used, but what a person wants is the result and then its
# warrant. A check that failed is not here at all — it stopped the install.
report_verified() {
...
# report_path is the one line here that is a problem rather than a result, so
# it is the one line that carries a colour.
#
...
# report_success <dir> <quarantining os> is everything a finished install
# says, in the order it is read: the mark, what happened, what warrants it,
# and what to do next.
#
...
install_main() {
	...
	*) die "unknown channel $CHANNEL: it is stable or next" ;;
	...
	os=$(detect_os) ||
		die "no release is built for $(uname -s); try \`npm i -g @missingstudio/eva\`"
	...
	# A local rehearsal build, reaching no network. Everything past this point
	# is the same for a release and for a rehearsal.
	if [ -n "$FROM_DIST" ]; then
		[ -d "$FROM_DIST" ] || die "$FROM_DIST is not a directory"

		archive=$(archive_name "$os" "$arch")
		[ -f "$FROM_DIST/$archive" ] ||
			die "$FROM_DIST holds no $archive
The archive names this script derives and the ones the build wrote disagree."

		report_start "from a local build" "$os/$arch $SEP $FROM_DIST"

		cp "$FROM_DIST/$archive" "$tmp/$archive"
		[ -f "$FROM_DIST/checksums.txt" ] ||
			die "$FROM_DIST has no checksums.txt, so nothing can be verified"

		# A rehearsal build is unsigned, and this says so rather than passing
		# over it. What the rehearsal exercises is the path, and the signature
		# is the one step of it that a release has and a laptop does not.
	...
	# The work this install does not have to do. It is asked after the version
	# is known and before anything is downloaded.
	...
	# The two small files ride inside the archive's transfer, so their
	# connection cost disappears into it. The seam stays one URL at a time,
	# which a curl given both URLs would not be, and which wget could not
	# serve.
	#
	# The bundle is optional, so a release published before signing began
	# still installs — on the checksum alone, and saying so. The file is
	# removed on a failed fetch because curl and wget both leave one behind,
	# and an empty file would reach cosign as a bundle rather than as an
	# absence.
...
Check the version, or the release page: https://github.com/$REPO/releases"
	...
	# The signature first, because it is what makes the checksums Eva's.
	# Checking the archive against a file of unknown origin and then asking
	# where the file came from would be doing the two in the order that proves
	# the least.
