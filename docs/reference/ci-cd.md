...
One consequence for `ci.yml`: its `verify` job does not run `bun audit`. A red
CI must mean the commit is wrong, and an advisory published overnight is not
the commit's doing — this file owns that verdict. The desk command `bun run
verify` keeps the audit step, because a person asking "is everything fine"
wants the whole answer.
