...
Named here so they are decided on purpose rather than by the first commit that needs them. Each is due before the stage that would harden a wrong shape. The full argument for each is the review that raised them, which also raised where the loop lives — now settled, and recorded above: [reviews/2026-08-09-architecture-review.md](reviews/2026-08-09-architecture-review.md).
