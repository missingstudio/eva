...
Four authorities govern this system, in order:

1. **The house design skills.** `landing-page-design` sets the visual system;
   `redesign-existing-projects` audits an existing surface against it.
2. **The [interface cheat sheet](https://interfaces.dev/cheat-sheet).** Adopted
...
3. **This file plus DESIGN.md.** They apply the rules above to missing studio.
4. **The surface.** A site implements; it never decides.
...
| --- | -------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| 1   | Fonts: Geist, Manrope, Geist Mono or Poppins | Space Grotesk                                                   | Self-hosted in `packages/brand`, and [brand.test.ts](../apps/docs/src/lib/brand.test.ts) fails the build on a third family or a CDN font. Space Grotesk is not on the forbidden list. | Swap the woff2 file and the two `@font-face` blocks. One commit, no rule change here.    |
...
| Stack                | TanStack Start, Tailwind v4, shared `@missingstudio/eva-brand`      |
...
Both surfaces consume [packages/brand](../packages/brand). Everything in
...
- Semantic names are the vocabulary. The CSS custom properties in
  [packages/brand/tokens.css](../packages/brand/tokens.css) are the
  implementation. The mapping tables below are the contract between them.
- A new token must be added to `packages/brand/tokens.css`, to DESIGN.md, and
...
It is authored once, in `packages/brand/tokens.css`, and deliberately left
...
- The theme control must reflect the resolved scheme and must not flash on
  ...
  [packages/brand/src/theme.ts](../packages/brand/src/theme.ts) owns this, and
...
- The product tagline is one string in
  [packages/brand/src/site.ts](../packages/brand/src/site.ts). Neither surface
...
- [ ] Every new token exists in `packages/brand/tokens.css` and in this file
