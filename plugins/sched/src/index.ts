...
const OPTIONS = declare({ serial: "list" })
 ...
 * one.
 */
export const serialNames = (options: Record<string, unknown>): readonly string[] =>
  OPTIONS.read(options, "serial", []).filter(
...
export const sched = define({
  ...
  effect: Effect.fn("eva.sched")(function* (ctx) {
    const serial = serialNames(ctx.options)
