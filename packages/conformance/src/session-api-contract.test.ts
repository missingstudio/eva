...
import { memorySessionAPI, type Answering } from "@missingstudio/eva-client-runtime"
import {
  ResumeTooFarBehind,
  WATCH_REPLAY_BOUND,
  type ModelRef,
...
/**
 * One suite, every filler of the Session API seam: the kernel's, which a
 * person really runs against; the in-memory one every suite that needs a
 * session drives instead; and the wire, which is how a browser reaches any of
 * them.
 *
...
/**
 * What each Run says: one entry per Run, as the scripted Provider serves one
 * per Provider Turn. A Run past the script says nothing and closes.
 */
const answering = (script: Script): Answering => {
  let served = 0
  return (_input, say) =>
    Effect.gen(function* () {
      const says = script[served] ?? []
      served += 1
      // One turn of the event loop before the first word, as the kernel's
      // filler takes when it forks the Run.
...
const memoryFiller: Filler = {
  ...
  with: (script, body) =>
    Effect.runPromise(
      Effect.scoped(
        Effect.flatMap(memorySessionAPI(answering(script), { model: FAKE_MODEL }), (memory) =>
          body(memory.api),
        ),
      ),
    ),
 ...
 * only ever ran on a socket a suite opened would prove the routes and not the
 * composition.
 */
const httpFiller: Filler = {
  name: "the wire in front of the in-memory one",
  with: (script, body) =>
    Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          const memory = yield* memorySessionAPI(answering(script), { model: FAKE_MODEL })

          // The address is read back out of what the surface printed, because
          // a `Frontend` carries none. Nothing here reads the page, so the
          // root names a tree no build ever filled.
          const said: string[] = []
          yield* serveWeb({
            root: join(tmpdir(), "eva-conformance-no-page"),
            bind: { host: "127.0.0.1", port: 0 },
            posture: "local",
            api: apiWire(memory.api),
          ...
          const over: SessionAPI = {
            ...
            submit: (session, input) =>
              Effect.flatMap(attached, () => transport.api.submit(session, input)),
            cancel: transport.api.cancel,
            answer: transport.api.answer,
          }
...
describe.each([kernelFiller, memoryFiller, httpFiller])(
  "the Session API, filled by $name",
  (filler) => {
