...
Four suites still call `boot` themselves, because they are not this shape:
`swap.test.ts` boots a kernel that loads nothing and adds plugins mid-Run,
`harness-host.test.ts` hands out a seam whose scope outlives the call, and
`both-doors.test.ts` and `two-surfaces.test.ts` hold a bound socket open
across the clauses that read and answer it.
...
| `two-surfaces.test.ts`         | One Session and two surfaces at once: the race between the doors, and the seam that is not there |
...
W2 exits the same way, on `w2-exit.test.ts`. A surface stage is a bold
paragraph under one roadmap heading rather than a heading of its own, so the
extractor takes the W2 slice first and reads the exit test inside it; the two
rules are the ones above. The two clauses the stage plan added — a retried
